...
def customer_transaction_history(customer): 
    ...
    return item_list, creation_date,ic,html
...
@frappe.whitelist()
def get_all_users():
    usernames = {i['username']:i['email'] for i in frappe.get_all("User", fields=['email','username'],filters={'username':["!=",'administrator']})}
    return list(usernames.keys()),usernames
...
def create_new_customer(data):
    ...
    customer.update({
        ...
        # 'state': data['state'],
        # 'city': data['district'],
        # 'pin_code_': data['pincode'],
        'name_of_mgr': data['name'],
    ...
    if('state' in key_list):customer.update({'state' : data['state']})
    if('district' in key_list):customer.update({'city':data['district']})
    if('pincode' in key_list):customer.update({'pin_code':data['pincode']})
    if('email' in key_list):customer.update({'email_of_mgr':data['email']})
    if('address1' in key_list):customer.update({'address_line_1':data['address1']})
    if('gstin' in key_list):customer.update({'gstin':data['gstin']})
    if('pan' in key_list):customer.update({'pan':data['pan']})
...
def validate_customer(data):
    ...
    key_list = list(data.keys())
    if(data['customer_type'] == 'Company'):
        if('gstin' not in key_list or 'pan' not in key_list):
            frappe.throw(_(
                "GSTIN and PAN is required for customer group Company"))
    return "true"
