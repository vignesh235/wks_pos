...
def get_user_name():
    return frappe.db.sql(
        ...
        select full_name
        from `tabUser`
        where full_name NOT IN ('Administrator', 'Guest')
        order by full_name
        LIMIT 0, 2000 """,
        as_dict=1,
    )
...
def get_customer_name_condition(pos_profile):
    condition =""
    if(pos_profile.get('customer')):
        condition += " AND name = '%s'"%(pos_profile.get('customer'))
    return condition
...
def get_customer_names(pos_profile):
    ...
    condition += get_customer_name_condition(pos_profile)
    customers = frappe.db.sql(
        """
        SELECT name, mobile_no, email_id, gstin, customer_name, address_line_1, address_line_2,
        state,city,pin_code_
        FROM `tabCustomer`
        WHERE {0}
        ORDER by name
        LIMIT 0, 10000 
...
@frappe.whitelist()
def get_pincode():
    pincode = frappe.db.sql(
        """
        SELECT ts_pincode
        FROM `tabTS Pincode`
        ORDER by ts_district
        LIMIT 0, 10000
        """,
        as_dict=1,
    )
...
@frappe.whitelist()
def update_invoice(data, cashier = None):
    ...
    if data.get("name"):
        ...
        # invoice_doc.owner=""
    if(cashier):
        invoice_doc.cashier = frappe.get_value('User', {'full_name':cashier}, 'email')
    ...
    invoice_doc.save()
    frappe.db.set_value('Sales Invoice', invoice_doc.name, 'grand_total',5*round((invoice_doc.grand_total+0.0001)/5))
...
def get_items_details(pos_profile, items_data):
    ...
    if len(items_data) > 0:
        for item in items_data:
            ...
            item_stock_qty = get_stock_availability(item_code, item.get('warehouse'))
            ...
            row.update(
                {
                    ...
                    "actual_qty": item_stock_qty or 150,
...
def create_customer(
    ...
    if not frappe.db.exists("Customer", {"customer_name": customer_name}):
        customer = frappe.get_doc(
            {
                ...
                "address_line_1":customer_address1,
        ...
        # if( customer_city and customer_address1 and not frappe.db.exists("Dyanmic Link", {"link_doctype":"Customer","link_name":customer.name,"link_title":customer.name, "parenttype": "Address"})):
        #     address=frappe.get_doc(
        #     {
        #         "doctype":"Address",           
        #         "address_line1":customer_address1,
        #         "city": customer_city,
        #         "links":[{"link_doctype":"Customer","link_name":customer.name,"link_title":customer.name}]     
        #     }
...
def get_items_from_barcode(selling_price_list, currency, barcode):
    ...
    if item_list[0]:
        ...
        return item
# @frappe.whitelist()
# def set_cashier(fieldname, customer, value=""):
#     field_with_doctype = {'cashier':'Sales Invoice'}
#     doctype = field_with_doctype[fieldname]
#     if(doctype == 'Sales Invoice'):
#         frappe.db.set_value("Sales Invoice", customer, "cashier", value)
#         frappe.db.commit() 
@frappe.whitelist()
def set_customer_info(fieldname, customer, value=""):
    frappe.errprint(fieldname)
    field_with_doctype = {'mobile_no':'Customer','phone':'Address','phone_no_optional':'Address',
    'phone_no1optional':'Address','address_line_1':'Customer','address_line_2':'Customer','territory':"Customer",
    'customer_group':'Customer','city':'Address','pin_code_':'Customer','customer_type':'Customer','gstin':'Customer','pan':'Customer','state':'Customer','territory':'Customer',}
    doctype = field_with_doctype[fieldname]
    ...
    if(doctype == 'Customer'):
        ...
        customer_doc.address_line_1=value
        # customer_doc.address_line_2=value
        frappe.errprint(fieldname)
        frappe.errprint(customer_doc.address_line_1)
        frappe.errprint(customer_doc.address_line_2)
        customer_doc.save()
        # if(fieldname == 'address_line_1'):
        #      frappe.db.set_value("Customer", customer, "address_line_1", value)
        #      frappe.db.commit()
         
        if(fieldname == 'pin_code_'):
             frappe.db.set_value("Customer", customer, "pin_code_", value)
             frappe.db.commit()  
        if(fieldname == 'address_line_2'):
             frappe.db.set_value("Customer", customer, "address_line_2", value)
             frappe.db.commit()
...
def customer_link_details():
    ...
    citys=frappe.get_all("TS District", pluck='name')
    
    state=frappe.get_all("TS State",pluck='name')
    types=['Individual', 'Company']
    pins=frappe.get_all("TS Pincode",pluck='name')
    # territory=frappe.get_all("Territory",pluck='Territory')
    return c_group, c_territory, citys, types, state, pins

@frappe.whitelist()
def customer_bls_details(cus):
    cus=frappe.get_all("Customer",filters={'customer_name':cus},fields=['state','city','pin_code_','pan','gstin','customer_type','mobile_no','address_line_1','address_line_2','territory']) 
    return cus 
