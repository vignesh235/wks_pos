      ...
      <v-row class="items px-2 py-1">
        <v-col class="pb-0 mb-1">
          <v-autocomplete
            ...
            color="primary"
            :label="frappe._('Select User')"
            :items="user"
            v-model="cashier"
            @change="group(cashier)"
            outlined
       >
      </v-autocomplete>
          <v-text-field
...
export default {
  ...
  methods: {
    group(cashier)
    {
      evntBus.$emit("get_cashier",cashier);
    },
    ...
    get_items() {
      ...
      frappe.call({
        ...
        callback: function (r) {
          if (r.message) {
            console.log(r.message.length, "LLLL")
            vm.items = r.message;
            evntBus.$emit('set_all_items', vm.items);
            vm.loading = false;
    ...
    pos_user_profile(){
      const vm = this;
      frappe.call({
          method: 'posawesome.posawesome.api.posapp.get_user_name',
          args: {},
          callback: function (r) {
            if (r.message) {
              r.message.forEach((element) => {
                vm.user.push(element.full_name);
              });
            }
          },
        });
